...
namespace AutoLead.Builders
{
	public class VehicleElementBuilder : BaseElementBuilder, IVehicleElementBuilder
	{
		...
		private IPriceElementBuilder PriceXElementBuilder { get; }
		...
		private XElement GetFinanceElement(Finance finance)
		{
			...
			if (finance.Amounts?.Count > 0)
			{
				foreach (var financeAmount in finance.Amounts)
				{
					...
					if (!string.IsNullOrEmpty(financeAmount.Currency))
					{
						financeAmountElement.Add(new XAttribute("currency", financeAmount.Currency));
					}
